    ...
    async def on_guild_join(self, guild: discord.Guild):
        ...
        async for entry in guild.audit_logs(limit=10, action=discord.AuditLogAction.bot_add):
            if entry.target.id == self.bot.user.id:
                ...
                break
        if inviter:
            if not get_user_data(0, inviter.id, "join_notify", False):
                return
            try:
                embed = discord.Embed(
                    title="欸？你好像邀請了我？",
            ...
            owner = guild.owner
            if not get_user_data(0, owner.id, "join_notify", False):
                return
            try:
                embed = discord.Embed(
                    title="欸？你好像邀請了我？",
                    description=f"你好啊，{owner.mention}！好像有人把我邀請進入你的伺服器 {guild.name} 了？但是我沒辦法私訊他 :/\n快速開始：\n- {get_command_mention('help')} 查看指令列表\n- {get_command_mention('tutorial')} 查看使用教學\n- {get_command_mention('panel')} 開啟網頁面板\n如果有任何問題，歡迎加入[支援伺服器]({config('support_server_invite')})尋求幫助！",
                    color=discord.Color.green()
                )
                embed.set_footer(text=guild.name, icon_url=guild.icon.url if guild.icon else None)
                website_button = discord.ui.Button(label="官方網站", url=config('website_url'))
