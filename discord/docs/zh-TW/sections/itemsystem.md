...
`content` 現在支援 AutoReply 模板語法，包含一般變數、`if` 條件、`math`、Embed 指令、`newmsg` / `edit` 等功能；語法檢查規則也比照 AutoReply，新增或編輯時若模板格式錯誤會直接擋下。

| 參數 | 說明 |
| --- | --- |
| `name` | 物品名稱（最長 100 字元） |
| `content` | 使用時傳送的文字內容（最長 2000 字元，可使用 AutoReply 變數） |
| `description` | 物品說明（可選） |
