...
| `/antibeast settings` | 設定連續觸發處置的啟用狀態、時間窗口、觸發次數、處置範圍與 Moderate.py action |
...
- 處置範圍：所有 AntiBeast 受保護提及

`action` 使用 Moderate.py 的動作字串格式，可使用 `kick`、`ban`、`mute` / `timeout` / `to`、`warn`、`send_mod_message` / `smm`、`force_verify` 等。也可以用逗號分隔最多 5 個動作。

如果開啟「僅處理提及 @everyone 或 @here 的成員」，未繞過身分組的 mention token 仍會被 AutoMod 阻擋，但不會計入連續觸發處置，也不會因此執行 Moderate.py action。
